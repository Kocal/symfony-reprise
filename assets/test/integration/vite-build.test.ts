...
describe('vite top-level input', () => {
    it.skipIf(!hasTopLevelInput)('emits Symfony files from the top-level input', async () => {
        const out = tmpDir('top-level-input');
        await build({
            root: fixture,
            logLevel: 'silent',
            input,
