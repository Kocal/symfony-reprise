...
- `assets/src/core/` — pure, no bundler imports: `options.ts` (`normalizeOptions` + CDN guard + `resolvePublicPath`), `dev-server.ts` (`resolveDevOrigin`), `format.ts` (`buildEntrypoints`/`buildManifest` in the frozen v1 format), `emit.ts` (`writeMetadata()` builds the context, the SRI hashes and both files, then writes them atomically; `watchIgnore()` gives watchers every path the build writes: `outputPath` and those metadata files).
...
- `assets/src/index.ts` — the `unpluginFactory` (Vite + Rsbuild) + `createUnplugin` default export. Universal `resolveId`/`load` serve the Stimulus virtual module for both bundlers. It holds the shared `isDev` state (set by either bundler, read by `load`) and plugs in the hook objects from `bundlers/`.
- `assets/src/bundlers/` — the per-bundler hook objects, in a subdirectory because tsdown publishes every top-level `src/*.ts` as an entry. `vite.ts` (`viteHooks`) calls the collectors + core: `config()` sets `base`/`outDir` and disables Vite's own manifest/publicDir copy; `generateBundle` collects the graph and `writeBundle` writes the two files into `metadataPath` on build; `configureServer` writes the dev-flavoured files pointing at the dev-server origin. `rsbuild.ts` (`rsbuildHooks`) does the same job for Rsbuild in `setup(api)` — `api.modifyRsbuildConfig` forces `tools.htmlPlugin = false` (no per-entry HTML), disables the public-dir copy (output lives under `public/build`), sets the output paths + dev origin, and extends Rspack's `watchOptions.ignored` with those paths through `mergeWatchIgnored`; `api.onAfterCreateCompiler` taps `compiler.hooks.done` to run `statsToGraph` + core. `@rsbuild/core` is an optional peer, so it is never imported (the `rspack` namespace is read off the compiler as `c.rspack`), which keeps it out of the Vite bundle.
